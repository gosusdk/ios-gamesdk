...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    ///
    NSError *error = nil;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"^(\\w+)@([a-z0-9]+)\\.\\w{1,3}$"
                                                                           options:NSRegularExpressionCaseInsensitive error:&error];
    
    NSString *email = @"abcxyz1212@yahooo.com";
    NSArray *matches = [regex matchesInString:email options:0 range:NSMakeRange(0, email.length)];
    
    long pos = [regex numberOfMatchesInString:email options:0 range:NSMakeRange(0, email.length)];
    if(pos > 0) {
        
    }
    
    NSLog(@"email regex pos = %ld",  pos);
    NSLog(@"matches = %@", matches);
    
    
    for (NSTextCheckingResult *match in matches)
    {
        NSString *matchText = [email substringWithRange:[match range]];
        NSLog(@"email regex = %@", matchText);
    }
